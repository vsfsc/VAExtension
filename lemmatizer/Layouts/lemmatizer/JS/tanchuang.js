...
Popup.prototype = {
    ...
    initialize: function (config) {
        this.config = Object.extend({ contentType: 1, isHaveTitle: true, scrollType: 'no', isBackgroundCanClick: false, isSupportDraging: true, isShowShadow: true, isReloadOnClose: false, width: 400, height: 300 }, config || {});
        ...
        this.color = { cColor: "#222", bColor: "#FFFFFF", tColor: "#777777", wColor: "#000" };
        /* cColor 修改阴影颜色 wColor 文本颜色*/
        this.dropClass = null;
        this.someToHidden = [];
    ...
    build: function () {
        ...
        var path = "images/";
        var close = '<input type="image" id="dialogBoxClose" src="' + path + 'dialogclose.gif" border="0" width="18" height="17" align="absmiddle" title="关闭"/>';
        var cB = 'filter: alpha(opacity=' + this.info.coverOpacity + ');opacity:' + this.info.coverOpacity / 100 + ';';
        var cover = '<div id="dialogBoxBG" style="position:absolute;top:0px;left:0px;width:100%;height:100%;z-index:' + baseZIndex + ';' + cB + 'background-color:' + this.color.cColor + ';display:none;"></div>';
        var mainBox = '<div id="dialogBox" style="border:10px solid ' + this.color.tColor + ';display:none;z-index:' + showZIndex + ';position:relative;width:' + this.config.width + 'px;"><table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="' + this.color.bColor + '">';
        if (this.config.isHaveTitle) {
            mainBox += '<tr height="24" bgcolor="' + this.color.tColor + '"><td><table style="-moz-user-select:none;height:24px;" width="100%" border="0" cellpadding="0" cellspacing="0" ><tr>' + '<td width="6" height="24" background="' + path + 'tab_table_bg_gray.gif" height="27"></td><td id="dialogBoxTitle" style="color:' + this.color.wColor + ';font-size:12px;font-weight:bold;padding-left:10px;" background="' + path + 'tab_table_bg_gray.gif" height="27">' + this.info.title + ' </td>' + '<td id="dialogClose" width="20" align="right" valign="middle" background="' + path + 'tab_table_bg_gray.gif" height="27">' + close + '</td><td width="6" background="' + path + 'tab_table_bg_gray.gif" height="27"></td></tr></table></td></tr>';
        }
        else {
...
function openshow(url, title, w, h, stype) {
    g_pop = new Popup({ contentType: stype, isReloadOnClose: true, width: w, height: h });
...
function Comment(url, w, h) {
    g_pop = new Popup({ contentType: 1, isReloadOnClose: true, width: w, height: h });
...
function ShowFloatDiV(url, title, w, h) {
    g_pop = new Popup({ contentType: 1, isReloadOnClose: true, width: w, height: h });
    g_pop.setContent("title", title);
    g_pop.setContent("scrollType", "no");
    g_pop.setContent("contentUrl", url);
